-- Export Sea reporting family (mode='sea'), mirroring report_expair_* with TEU + CBM.
-- TEU = per DISTINCT consol (avoids LCL house over-count); CBM = house sum(volume_m3).
-- Optional p_load_type ('FCL'|'LCL'|NULL). Functions kept SECURITY INVOKER so they inline.

-- RLS perf: make the containers staff-check a single STABLE InitPlan so it short-circuits
-- the per-row own_containers EXISTS for staff (this is what unblocked the report timeouts).
alter policy staff_all_containers on public.containers using (is_staff());
alter policy own_containers on public.containers
  using (exists (
    select 1 from public.shipments sh
    where sh.consol_key = containers.consol_key
      and sh.customer_account_id in (
        select portal_users.account_id from public.portal_users
        where portal_users.user_id = (select auth.uid())
      )
  ));

create or replace function public.report_expsea_trend(
...
language sql stable as $$
  with base as (
    ...
    where s.direction='export' and s.mode='sea'
...
create or replace function public.report_expsea_lanes(
  p_from date, p_to date, p_destination text default null,
...
create or replace function public.report_expsea_parties(
  p_from date, p_to date, p_destination text default null,
  p_customer text default null, p_consignee text default null,
  p_load_type text default null, p_limit integer default 50)
...
create or replace function public.report_expsea_destinations(p_from date, p_to date, p_load_type text default null)
...
create or replace function public.report_expsea_customers(p_from date, p_to date, p_load_type text default null)
...
create or replace function public.report_expsea_consignees(p_from date, p_to date, p_load_type text default null)
...
grant execute on function public.report_expsea_trend(date,date,text,text,text,text) to authenticated;
grant execute on function public.report_expsea_lanes(date,date,text,text,text,text,integer) to authenticated;
grant execute on function public.report_expsea_parties(date,date,text,text,text,text,integer) to authenticated;
grant execute on function public.report_expsea_destinations(date,date,text) to authenticated;
grant execute on function public.report_expsea_customers(date,date,text) to authenticated;
grant execute on function public.report_expsea_consignees(date,date,text) to authenticated;
