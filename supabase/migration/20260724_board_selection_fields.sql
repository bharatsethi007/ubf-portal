-- delivery_mode for import sea ops + board row fetch fields
ALTER TABLE public.bookings
  ADD COLUMN IF NOT EXISTS delivery_mode text;

ALTER TABLE public.bookings
  DROP CONSTRAINT IF EXISTS bookings_delivery_mode_check;

ALTER TABLE public.bookings
  ADD CONSTRAINT bookings_delivery_mode_check
  CHECK (delivery_mode IS NULL OR delivery_mode IN ('devan_yard', 'direct_importer'));
...
CREATE OR REPLACE FUNCTION public.get_import_sea_board()
 RETURNS TABLE(
  ...
  portconnect_enabled boolean, delivery_mode text
 )
 LANGUAGE sql
 STABLE
AS $function$
  SELECT
    ...
    COALESCE(bt.portconnect_enabled, false) AS portconnect_enabled,
    b.delivery_mode
...
CREATE OR REPLACE FUNCTION public.get_import_sea_board_row(p_booking_id uuid)
 RETURNS TABLE(
  id uuid, booking_ref text, job_no text, customer_id text, customer_name text,
  eta date, eta_source text, atf text, atf_source text, shipping_line text,
  shipping_line_source text, discharge_port text, discharge_port_source text,
 ...
 STABLE
AS $function$
  SELECT * FROM public.get_import_sea_board() board WHERE board.id = p_booking_id;
$function$;
