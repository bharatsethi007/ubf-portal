// STAFF (verify_jwt = true). Parses an uploaded AIR tariff sheet into structured lane lines via Claude.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: card, error: cardErr } = await db
      .from("rate_cards").select("currency_code, vendor_account_id, vendor_name, valid_from, valid_to")
    ...
    const userContent = buildUserContent({ sheet, card, rulesText })
