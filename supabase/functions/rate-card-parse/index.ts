// STAFF (verify_jwt = true). Parses an uploaded rate-card sheet into structured FCL lane lines via Claude.
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    const { data: card, error: cardErr } = await db
      .from("rate_cards").select("currency_code, shipping_line_code, valid_from, valid_to")
    ...
    const { data: vendorRule } = await db.from("rate_rules").select("content")
      .eq("scope", "vendor").eq("vendor_kind", "shipping_line").eq("vendor_code", card.shipping_line_code).maybeSingle()
    const rulesText = [globalRule?.content, vendorRule?.content].filter(Boolean).join("\n\n")

    const { data: containers } = await db.from("container_types").select("code, label").eq("active", true).order("sort_order")
    ...
    const userContent = buildUserContent({
      sheet, card, rulesText, containers: containers ?? [], ports: ports ?? [], aliases: aliases ?? [],
    })
