...
export default function DriverRail({ drivers, selectedId, onSelect, onDropCard }: Props) {
  ...
  return (
    <div className="flex w-44 shrink-0 flex-col gap-1 overflow-y-auto border-r border-neutral-200 pr-2">
      ...
      {drivers.map((d) => {
        ...
        return (
          <button key={d.id} type="button" onClick={() => onSelect(on ? null : d.id)}
            ...
            className={`flex items-center gap-2 rounded-md px-2 py-1.5 text-left ${dropping ? 'ring-2 ring-[#0A2472] bg-[#0A2472]/5' : on ? 'bg-[#0A2472]/10 ring-1 ring-[#0A2472]/30' : 'hover:bg-neutral-50'}`}>
            <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#0A2472] text-[10px] font-semibold text-white">{(d.first_name[0] ?? '') + (d.last_name[0] ?? '')}</span>
