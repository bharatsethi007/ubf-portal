...
export default function TruckMap({ routeDriverId = null, driverName = null, onTruckClick, drivers = [], onAssignJob }: Props) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      map.addLayer({ id: 'route-done-arrows', type: 'symbol', source: 'route-done', layout: { 'symbol-placement': 'line', 'symbol-spacing': 60, 'text-field': '→', 'text-size': 16, 'text-font': ['DIN Pro Bold', 'Arial Unicode MS Bold'], 'text-keep-upright': false, 'text-rotation-alignment': 'map', 'text-pitch-alignment': 'viewport', 'text-allow-overlap': true, 'text-ignore-placement': true }, paint: { 'text-color': '#F26A21', 'text-halo-color': '#ffffff', 'text-halo-width': 1.5 } })
      map.addLayer({ id: 'route-todo-arrows', type: 'symbol', source: 'route-todo', layout: { 'symbol-placement': 'line', 'symbol-spacing': 60, 'text-field': '→', 'text-size': 16, 'text-font': ['DIN Pro Bold', 'Arial Unicode MS Bold'], 'text-keep-upright': false, 'text-rotation-alignment': 'map', 'text-pitch-alignment': 'viewport', 'text-allow-overlap': true, 'text-ignore-placement': true }, paint: { 'text-color': '#DC2626', 'text-halo-color': '#ffffff', 'text-halo-width': 1.5 } })
