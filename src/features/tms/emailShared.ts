// Shared chrome for TMS document emails (pickup docs + POD). Pure functions, no deps.
// One place for the header/logo, brand colours, party boxes, manifest, and the HTML shell.

export const NAVY = '#0A2472'
export const ORANGE = '#f5880d'
// Hosted reversed (white-letter) logo on transparent bg (src/assets/ub-logo-email.png).
// For production stability this can later move to a Pages repo or Supabase public bucket.
export const LOGO_URL = 'https://raw.githubusercontent.com/bharatsethi007/ubf-portal/main/src/assets/ub-logo-email.png'

export type EmailParty = { company: string; address_lines: string[]; contact?: string; phone?: string; reference?: string }
export type EmailCargoLine = {
  qty: number; description: string
  length_m?: number | null; width_m?: number | null; height_m?: number | null; volume_m3?: number | null; weight_kg?: number | null
}
...
export function partyBox(label: string, p: EmailParty): string {
...
/** Two-party row (e.g. Sender/Receiver, or Delivered to/Sender). */
export function partiesRow(leftLabel: string, left: EmailParty, rightLabel: string, right: EmailParty): string {
  return `
        <tr><td class="px" style="padding:24px 32px 0 32px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
            <td class="stack" valign="top" width="49%" style="width:49%; border:1px solid #dcdfe4;">${partyBox(leftLabel, left)}</td>
            <td class="stack stack-gap" width="2%" style="width:2%; font-size:0; line-height:0;">&nbsp;</td>
            <td class="stack" valign="top" width="49%" style="width:49%; border:1px solid #dcdfe4;">${partyBox(rightLabel, right)}</td>
          </tr></table>
        </td></tr>`
}

/** Consignment / <secondary> header row (secondary = Issued date or Delivered date). */
export function idRow(consignmentNo: string, secondaryLabel: string, secondaryValue: string): string {
  return `
        <tr><td class="px" style="padding:28px 32px 0 32px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
            <td align="left" valign="bottom">
              ...
              <div class="num" style="font-size:26px; font-weight:700; color:${NAVY}; letter-spacing:0.5px; padding-top:4px;">${esc(consignmentNo)}</div>
            </td>
            <td align="right" valign="bottom">
              <div style="font-size:11px; font-weight:600; letter-spacing:1.5px; color:#8a93a6; text-transform:uppercase;">${esc(secondaryLabel)}</div>
              <div style="font-size:14px; font-weight:600; color:#2b3242; padding-top:6px;">${esc(secondaryValue)}</div>
            </td>
          </tr></table>
        </td></tr>`
}

export function introRow(text: string): string {
  return `
        <tr><td class="px" style="padding:20px 32px 0 32px; font-size:14px; line-height:22px; color:#3a4252;">${text}</td></tr>`
}

/** PDF attachment chips row. */
export function attachmentChips(names: string[]): string {
  const cells = names.map((nm, i) => `${i ? '<td style="width:10px; font-size:0;">&nbsp;</td>' : ''}<td valign="middle" style="padding:8px 14px; border:1px solid #dcdfe4; background-color:#f6f7f9; font-size:12px; font-weight:600; color:#2b3242;"><span style="color:${NAVY};">PDF</span>&nbsp;&nbsp;${esc(nm)}</td>`).join('')
  return `
...
/** Small detail strip of up to 3 labelled cells (e.g. Delivered at / Driver / Pieces). */
export function detailStrip(cells: { k: string; v: string }[]): string {
  const c = cells.map((x, i) => `<td valign="top" width="${Math.floor(100 / cells.length)}%" style="padding:11px 14px; ${i ? 'border-left:1px solid #dcdfe4;' : ''}"><div style="font-size:9px; font-weight:700; letter-spacing:1px; color:#8a93a6; text-transform:uppercase;">${esc(x.k)}</div><div style="font-size:13px; font-weight:600; color:#1c2233; padding-top:4px;">${esc(x.v)}</div></td>`).join('')
  return `
        <tr><td class="px" style="padding:20px 32px 0 32px;">
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #dcdfe4;"><tr>${c}</tr></table>
        </td></tr>`
}

/** Manifest table (QTY / DESCRIPTION / L / W / H / V / KG + totals). */
export function manifestSection(cargo: EmailCargoLine[]): string {
  ...
  const rows = cargo.map((c) => `
    <tr>
      ...
      <td style="${td}">${n2(c.length_m)}</td><td style="${td}">${n2(c.width_m)}</td><td style="${td}">${n2(c.height_m)}</td><td style="${td}">${n2(c.volume_m3)}</td>
  ...
  const totQty = cargo.reduce((t, c) => t + (Number(c.qty) || 0), 0)
  const totVol = cargo.reduce((t, c) => t + (Number(c.volume_m3) || 0), 0)
  const totKg = cargo.reduce((t, c) => t + (Number(c.weight_kg) || 0), 0)
  return `
        <tr><td class="px" style="padding:24px 32px 0 32px;">
          ...
          <table role="presentation" width="100%" cellpadding="0" cellspacing="0" border="0" style="border:1px solid #dcdfe4;">
            <tr>
              ...
              <td style="${hth}">L</td><td style="${hth}">W</td><td style="${hth}">H</td><td style="${hth}">V&nbsp;m&sup3;</td><td style="${hth} padding-right:10px;">KG</td>
            </tr>
            ${rows}
            <tr>
...
/** Full HTML document shell: navy logo header + orange rule + innerHtml + UB Freight footer. */
export function wrapEmailDocument(opts: { title: string; preheader: string; innerHtml: string }): string {
  return `<!DOCTYPE html>
...
<body style="margin:0; padding:0; background-color:#eceef1;">
  <div style="display:none; max-height:0; overflow:hidden; mso-hide:all; font-size:1px; line-height:1px; color:#eceef1; opacity:0;">${esc(opts.preheader)}</div>
