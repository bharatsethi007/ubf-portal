/** Pill suffix: G general, H high-cube, R reefer. */
...
function normalizeAlphanumeric(code: string): string | null {
  ...
  const isHighCubeLength = m[1].toUpperCase() === '45'
  if (isHighCubeLength && kind === 'G') return `${size}H`
  return `${size}${kind}`
}
...
/**
 * Normalise ISO 6346 size/type (numeric e.g. 2210 or alphanumeric e.g. 22G1)
 * to a short pill label (20G, 40H, 20R, …). Falls back to description, then raw code.
 */
