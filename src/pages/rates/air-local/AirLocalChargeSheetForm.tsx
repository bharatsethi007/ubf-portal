...
import { createAirLocalChargeSheet } from './airLocalChargesApi'
...
export default function AirLocalChargeSheetForm() {
  ...
  async function onSubmit() {
    ...
    setError('')
    try {
      const { id } = await createAirLocalChargeSheet({ direction, movement, title: title.trim(), valid_from: validFrom, valid_to: validTo })
      navigate(`/setup/rates/air-local/${id}`)
    } catch (e) {
    ...
    <div className="quotes-page">
      <div className="card quotes-page__card">
        <header className="quotes-page__head">
          ...
          <h1>New air local charge sheet</h1>
          <p style={{ margin: '4px 0 0', color: 'var(--muted-foreground)', fontSize: 14 }}>
            Set the sheet identity. You&apos;ll add airports, airlines, and charge lines next.
          </p>
        </header>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 20, alignItems: 'center' }}>
          ...
          <Link to="/setup/rates/air-local" style={{ display: 'inline-flex', alignItems: 'center', padding: '8px 14px', fontSize: 14, color: 'var(--muted-foreground)', textDecoration: 'none' }}>Cancel</Link>
        </div>
      </div>
    </div>
  )
