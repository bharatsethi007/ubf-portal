...
import { listFclRateCards, type FclRateCardRow } from '../ratesApi'
import { fclRateCardsColumns, FCL_STATUS_TABS } from './fclRateCardsColumns'

const PAGE_SIZE = 50

export default function FclRateCardsList() {
  const navigate = useNavigate()
  const [search, setSearch] = useState('')
  const [statusTab, setStatusTab] = useState<string>('all')
  const [page, setPage] = useState(1)
  const [rows, setRows] = useState<FclRateCardRow[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const debouncedSearch = useDebouncedValue(search, 300)
  const columns = useMemo(() => fclRateCardsColumns(), [])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const res = await listFclRateCards({ page, pageSize: PAGE_SIZE, search: debouncedSearch, status: statusTab })
        if (cancelled) return
        setRows(res.rows)
        setTotal(res.total)
        setError('')
    ...
    <div className="quotes-page">
      <div className="card quotes-page__card">
        ...
        <div className="quotes-page__toolbar">
          ...
          <button type="button" className="btn quotes-page__new-btn" onClick={() => navigate('/setup/rates/fcl/new')}>
