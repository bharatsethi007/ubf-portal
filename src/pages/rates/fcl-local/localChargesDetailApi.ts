...
export type LocalChargeSheetDetail = {
  id: string
  title: string | null
  direction: string
  movement: string
...
export type LocalChargeHeaderPatch = {
  title: string | null
  direction: string
  movement: string
  port_codes: string[]
...
export async function updateLocalChargeSheetHeader(id: string, p: LocalChargeHeaderPatch): Promise<void> {
...
export type LocalChargeLineDraft = {
  key: string
  dbId: string | null
  charge_code: string
  label: string
...
export async function listLocalChargeLines(sheetId: string): Promise<LocalChargeLineDraft[]> {
  const { data, error } = await supabase
    .from('local_charge_lines')
    .select('id, charge_code, label, container_types, basis, buy_amount, buy_currency, sell_amount, sell_currency, min_buy, min_sell, vendor_account_id, vendor_name, condition')
    .eq('sheet_id', sheetId)
  ...
  return ((data as Record<string, any>[]) ?? []).map((r) => ({
    ...
    container_types: Array.isArray(r.container_types) ? r.container_types.map(String) : [],
...
export async function saveLocalChargeLines(
  sheetId: string,
  ...
  if (toDelete.length) {
    const { error } = await supabase.from('local_charge_lines').delete().in('id', toDelete)
    if (error) throw error
  }
  ...
  for (const r of rows) {
    ...
    if (r.dbId) {
      const { error } = await supabase.from('local_charge_lines').update(payload).eq('id', r.dbId)
      if (error) throw error
    } else {
      const { error } = await supabase.from('local_charge_lines').insert(payload)
      if (error) throw error
    }
  }
}
