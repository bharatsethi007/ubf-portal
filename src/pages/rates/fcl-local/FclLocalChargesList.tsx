...
export default function FclLocalChargesList() {
  ...
  const [rows, setRows] = useState<LocalChargeSheetRow[]>([])
  const [total, setTotal] = useState(0)
  const [loading, setLoading] = useState(true)
  const [error, setError] = useState('')
  const debouncedSearch = useDebouncedValue(search, 300)
  const columns = useMemo(() => localChargeSheetsColumns(), [])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        setRows([])
        setTotal(0)
        setError(e instanceof Error ? e.message : 'Failed to load local charge sheets')
      } finally {
    ...
    <div className="quotes-page">
      <div className="card quotes-page__card">
        ...
        <div className="quotes-tabs" role="tablist" aria-label="Sheet status">
          {LOCAL_STATUS_TABS.map(({ key, label }) => (
