...
let tmpSeq = 0
export function newLclSurcharge(defaultCurrency: string): FclSurchargeDraft {
  tmpSeq += 1
  return { key: `tmp-${tmpSeq}`, dbId: null, charge_code: '', label: '', amount: '', sell_amount: '', currency_code: defaultCurrency, basis: 'per_cbm', scope: 'dest', container_type: '', condition: '', charge_group: '' }
}

type Props = { rows: FclSurchargeDraft[]; defaultCurrency: string; onChange: (rows: FclSurchargeDraft[]) => void }

export default function LclSurchargesGrid({ rows, defaultCurrency, onChange }: Props) {
  ...
  function add() { onChange([...rows, newLclSurcharge(defaultCurrency)]) }
