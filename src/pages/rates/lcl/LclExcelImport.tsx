...
import { insertLclLines, learnPortAliases, type LclLineDraft } from '../ratesApi'
...
function pickDefaultSheet(names: string[]): string {
  const score = (n: string) => {
    ...
    if (/tariff|rate|freight/.test(s)) v += 2
...
export default function LclExcelImport({ cardId, defaultCurrency, onImported }: Props) {
  const [sheets, setSheets] = useState<SheetData[]>([])
  const [selectedSheet, setSelectedSheet] = useState('')
  const [fileName, setFileName] = useState('')
  const [open, setOpen] = useState(false)
  const [draft, setDraft] = useState<LclLineDraft[]>([])
  ...
  async function parseWithAI() {
    ...
    setParsing(true)
    try {
      const { data, error } = await supabase.functions.invoke('rate-card-parse-lcl', {
      ...
      const drafts: LclLineDraft[] = lines.map((l, i) => ({
        key: `ai-${i}`, dbId: null,
        origin_port_code: l.origin_port_code || '',
        dest_port_code: l.dest_port_code || '',
        rate_per_wm: l.rate_per_wm != null ? String(l.rate_per_wm) : '',
        ...
        lane_charges: Array.isArray(l.lane_charges)
          ? l.lane_charges.map((c: any) => ({ code: String(c.code || '').toUpperCase(), label: String(c.label || c.code || ''), per_wm: Number(c.per_wm) || 0 }))
          : [],
  ...
  async function save() {
    ...
    const bad = draft.filter((l) => !l.origin_port_code || !l.dest_port_code || l.rate_per_wm === '' || isNaN(Number(l.rate_per_wm)))
    if (bad.length) { toast.error(`${bad.length} line${bad.length === 1 ? '' : 's'} still need an origin, destination and numeric rate — fix the “—” rows before saving`); return }
    setSaving(true)
    try {
      await insertLclLines(cardId, draft)
      ...
      try {
        ...
        const pairs = draft
          ...
          .filter((p) => { const raw = p.alias.toLowerCase(); return raw && raw !== p.port_code.toLowerCase() && raw !== portByCode.get(p.port_code) })
          ...
          <div style={{ width: '90vw', height: '90vh', maxWidth: 1600, background: '#fff', borderRadius: 14, display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.3)' }}>
            ...
            <div style={{ flex: 1, overflow: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 20 }}>
              ...
              <section>
                ...
                <LclLinesGrid lines={draft} defaultCurrency={defaultCurrency} onChange={setDraft} />
              </section>
            </div>
