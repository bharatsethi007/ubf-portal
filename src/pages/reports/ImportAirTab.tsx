...
import { NAVY, BLUE, C, FONT, glass, nf, cf, Card, Title, LegendDot, KpiRail, Td, Seg, SearchSelect, type Opt } from './reportsUi'
...
const NUMERIC = new Set(['masters', 'houses', 'gross_kg', 'chargeable_kg', 'revenue'])
function sortRows<T>(rows: T[], key: keyof T, dir: SortDir): T[] {
  const numeric = NUMERIC.has(key as string)
  const arr = [...rows]
  arr.sort((a, b) => {
    const c = numeric
      ? num(a[key]) - num(b[key])
      : String((a[key] as any) ?? '').localeCompare(String((b[key] as any) ?? ''))
    return dir === 'asc' ? c : -c
  })
  return arr
}
function SortTh({ label, active, dir, right, onClick }: { label: string; active: boolean; dir: SortDir; right?: boolean; onClick: () => void }) {
  return (
...
export default function ImportAirTab() {
  ...
  const [laneSort, setLaneSort] = useState<{ key: keyof LaneRow; dir: SortDir }>({ key: 'houses', dir: 'desc' })
  const [partySort, setPartySort] = useState<{ key: keyof PartyRow; dir: SortDir }>({ key: 'houses', dir: 'desc' })
  ...
  function sortLane(key: keyof LaneRow) {
    setLaneSort((s) => (s.key === key ? { key, dir: s.dir === 'asc' ? 'desc' : 'asc' } : { key, dir: NUMERIC.has(key as string) ? 'desc' : 'asc' }))
    setLanePage(1)
  }
  function sortParty(key: keyof PartyRow) {
    setPartySort((s) => (s.key === key ? { key, dir: s.dir === 'asc' ? 'desc' : 'asc' } : { key, dir: NUMERIC.has(key as string) ? 'desc' : 'asc' }))
    setPartyPage(1)
  }
  ...
  useEffect(() => {
    ...
    ;(async () => {
      const [de, cu, co] = await Promise.all([
        supabase.rpc('report_impair_destinations', { p_from: from, p_to: to }),
        supabase.rpc('report_impair_customers', { p_from: from, p_to: to }),
        supabase.rpc('report_impair_consignees', { p_from: from, p_to: to }),
      ])
      if (cancelled) return
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      const [tr, ln, pa] = await Promise.all([
        supabase.rpc('report_impair_trend', args),
        supabase.rpc('report_impair_lanes', { ...args, p_limit: 300 }),
        supabase.rpc('report_impair_parties', { ...args, p_limit: 300 }),
      ])
      if (cancelled) return
  ...
  const sortedLanes = useMemo(() => sortRows(lanes, laneSort.key, laneSort.dir), [lanes, laneSort])
  const sortedParties = useMemo(() => sortRows(parties, partySort.key, partySort.dir), [parties, partySort])
  const laneSlice = sortedLanes.slice((lanePage - 1) * PAGE_SIZE, lanePage * PAGE_SIZE)
  const partySlice = sortedParties.slice((partyPage - 1) * PAGE_SIZE, partyPage * PAGE_SIZE)
  ...
  const laneCols: { key: keyof LaneRow; label: string; right?: boolean }[] = [
    { key: 'origin', label: 'Origin' }, { key: 'destination', label: 'Dest' },
    { key: 'masters', label: 'Master', right: true }, { key: 'houses', label: 'House', right: true },
    { key: 'chargeable_kg', label: 'Chg kg', right: true }, { key: 'revenue', label: 'Revenue', right: true },
  ]
  const partyCols: { key: keyof PartyRow; label: string; right?: boolean }[] = [
    { key: 'customer_name', label: 'Customer' }, { key: 'consignee_name', label: 'Consignee' },
    { key: 'masters', label: 'Master', right: true }, { key: 'houses', label: 'House', right: true },
    { key: 'chargeable_kg', label: 'Chg kg', right: true }, { key: 'revenue', label: 'Revenue', right: true },
  ]
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card pad={0}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <thead><tr>
              {laneCols.map((col) => (
                <SortTh key={col.key as string} label={col.label} right={col.right} active={laneSort.key === col.key} dir={laneSort.dir} onClick={() => sortLane(col.key)} />
              ))}
            </tr></thead>
            <tbody>
              {laneSlice.map((r, i) => (
        ...
        <Card pad={0}>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', tableLayout: 'fixed' }}>
            ...
            <thead><tr>
              {partyCols.map((col) => (
                <SortTh key={col.key as string} label={col.label} right={col.right} active={partySort.key === col.key} dir={partySort.dir} onClick={() => sortParty(col.key)} />
              ))}
            </tr></thead>
            <tbody>
              {partySlice.map((r, i) => (
